...
<head>
    ...
    <title>Selamat Datang di FOOD+</title>
...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const totalRestoranEl = document.getElementById("total-restoran");

            // Fetch summary stats
            fetch("/api/statistics/receiver/dashboard/summary")
                .then(response => response.json())
                .then(({ data }) => {
                    totalMakananEl.textContent = data.total_donation || 0;
                    totalRestoranEl.textContent = data.total_resto || 0;
                })
                .catch(error => console.error('Error fetching stats:', error));
            
            // Fetch all restaurants/donations
            fetch("http://localhost:8000/api/restos/all")
                .then(response => response.json())
                .then(({ data }) => {
                    donationContainer.innerHTML = ""; // Clear the spinner/loading
                    if (!data || !data.length) {
                        donationContainer.innerHTML = `<p class="col-span-full text-center text-text-light">Belum ada donasi yang tersedia saat ini.</p>`;
                        return;
                    }

                    data.forEach(resto => {
                        // Use a placeholder image if the actual one is not available
                        const imageUrl = `https://placehold.co/600x400/2d7d8a/FFFFFF?text=${resto.name.charAt(0)}`;

                        const donationCardHTML = `
                            <div class="restaurant-card">
                                <img src="${imageUrl}" alt="Gambar makanan dari ${resto.name}" class="restaurant-card-img">
                                <div class="restaurant-card-body">
                                    <h4 class="font-bold text-lg">${resto.name}</h4>
                                    <div class="flex flex-wrap gap-2">
                                        <span class="tag">Makanan Siap Saji</span>
                                        <span class="tag">Minuman</span>
                                    </div>
                                    <div class="restaurant-stats">
                                        <i class="fas fa-heart text-red-500"></i> ${resto.likes_count || 0} Suka · 
                                        <i class="fas fa-comments text-primary-color"></i> ${resto.comments_count || 0} Ulasan
                                    </div>
                                </div>
                                <div class="card-footer-action">
                                     <a href="/login" class="block w-full text-center py-2 px-4 rounded-lg btn-secondary">
                                        Login untuk Lihat Detail
                                     </a>
                                </div>
                            </div>
                        `;
                        donationContainer.innerHTML += donationCardHTML;
                    });
                })
                .catch(error => {
                    console.error('Error fetching donations:', error);
                    donationContainer.innerHTML = `<p class="col-span-full text-center text-red-500">Gagal memuat data donasi. Silakan coba lagi nanti.</p>`;
                });
        });
    </script>
</body>
